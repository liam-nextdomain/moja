...
| 경로 | 결과 | 비고 |
|---|---|---|
| 카카오톡 파일 공유 | 유지됨 | 검증일 2026-09-09 |
| 슬랙 파일 공유 | 유지됨 | 검증일 2026-09-09 |
| 구글 드라이브 업로드 | 깨짐 | 검증일 2026-09-09 |
| 원드라이브 동기화 | 미검증 | |
...
**깨짐**은 전송 경로 어딘가에서 이름이 다시 분해된 경우, **미검증**은 아직 확인하지 않은 경우다.
"변환됨" 같은 표현은 앱이 고친 것인지 전송 경로가 망가뜨린 것인지 반대로 읽히므로 쓰지 않는다.

메일 첨부는 위 표에 한 줄로 기록할 수 없다. 첨부 파일 이름을 MIME 헤더에 인코딩하는 주체는 보내는
클라이언트이고, 그 이름을 보관했다가 내려받을 때 다시 내보내는 주체는 받는 메일 서비스이므로, 두
사업자의 조합마다 결과가 달라진다. 카카오톡이나 슬랙은 보내는 쪽과 받는 쪽이 같은 사업자라서 이
문제가 없고, 서로 다른 사업자를 거치는 메일에만 해당한다. 따라서 조합마다 따로 검증해서 기록하고,
한 조합이 정상이었다는 이유로 다른 조합까지 정상이라고 판단하지 않는다.

| 보내는 쪽 | 받는 쪽 | 결과 | 비고 |
|---|---|---|---|
| 네이버 메일 (웹) | 네이버 메일 | 유지됨 | 검증일 2026-09-09 |
| 네이버 메일 (웹) | 지메일 | 깨짐 | 검증일 2026-09-09 |
| 지메일 (웹) | 지메일 | 깨짐 | 검증일 2026-09-09 |
| 지메일 (웹) | 네이버 메일 | 유지됨 | 검증일 2026-09-09 |
| macOS 기본 메일 앱 | 네이버 메일 | 유지됨 | 검증일 2026-09-09 |
| macOS 기본 메일 앱 | 지메일 | 유지됨 | 검증일 2026-09-09 |

받는 쪽을 웹 브라우저로 열었는지 메일 앱으로 수신했는지는 이 검증에서 기록하지 않았다. 아래 문단이
요구하는 축이므로, 다시 검증할 때 비고에 채운다.

받는 쪽이 웹 브라우저로 내려받는지 아웃룩 같은 메일 앱으로 수신하는지에 따라서도 결과가 갈릴 수
있다. 이 축까지 표의 열로 쪼개면 항목이 지나치게 늘어나므로, 검증할 때 어느 쪽이었는지를 비고에
적는다.
...
3. **일시정지 시 재개 동작**: 재개 시 전체 재스캔이 기본인지, 재개 이후 이벤트만 볼지.
...
지원하지 않는 볼륨은 "이 디스크(exFAT)는 변환을 지원하지 않습니다"로 표시하고 건너뛴다.
...
### 12.4a T1의 "2초 안에"는 FR-2와 양립할 수 없다
...
"이 디스크(exFAT)는 변환을 지원하지 않습니다"로 표시하고 건너뛴다.
