...
- 측정일: 기록되지 않음 (v0.1.0 배포 전). `파인더 기본 압축 (zip)` 한 줄만 2026-09-12에 따로 측정했다
...
**zip은 받는 쪽이 어느 도구로 푸느냐에 따라 갈린다.** 위 `유지됨`은 반디집으로 압축을
풀어서 얻은 결과다. 같은 압축 파일을 Windows 기본 압축 풀기에 넘겼을 때에는 압축 해제
자체가 실패해서, 이름이 어떻게 보이는지 확인하는 단계까지 가지도 못했다. 그러므로 이
줄은 "zip이 조합형 이름을 그대로 실어 나른다"는 사실까지만 말해 준다.

실패한 원인은 규명하지 않았다. 압축 파일의 형식 때문인지 그 Windows 기기에 국한된
문제인지 가려내지 않았고, 이 표의 다른 줄들과 마찬가지로 확인한 횟수도 한 번뿐이다.
