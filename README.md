...
**맥에서 만든 `보고서.docx`가 Windows에서 `ㅂㅗㄱㅗㅅㅓ.docx`로 깨져 보입니다.**

**모자는 폴더를 지켜보다가, 그런 이름이 생기면 바로 되돌려 놓습니다.**

메뉴바에 조용히 상주하는 작은 앱입니다. 서버 없음, 로그인 없음, 인터넷 통신 없음.
...
> **아직 정식 릴리스 전입니다 (v0.1.0).**
> 기능은 다 들어갔고 수용 기준 16개 중 12개를 자동 검증으로 통과했습니다.
> 남은 항목은 [릴리스까지 남은 일](#릴리스까지-남은-일)에 적어 두었습니다.

정식 배포가 시작되면 [Releases](../../releases)에서 `Moja-x.y.z.zip`을 내려받습니다.

## 설치 (3단계)

모자는 아직 Apple의 공증을 받지 않아서, 처음 열 때 한 단계가 더 필요합니다.
한 번만 하면 됩니다.

**1. 압축을 풀고 `Moja.app`을 응용 프로그램 폴더로 옮깁니다.**

<!-- 스크린샷 자리: Moja.app을 응용 프로그램 폴더로 끌어다 놓는 모습 -->

**2. 더블클릭하면 "확인되지 않은 개발자" 경고가 나옵니다. 일단 닫습니다.**

<!-- 스크린샷 자리: "Apple이 악성 소프트웨어 여부를 확인할 수 없습니다" 경고창 -->

**3. 시스템 설정 → 개인정보 보호 및 보안 → 아래로 내려 "그래도 열기"를 누릅니다.**

<!-- 스크린샷 자리: 시스템 설정의 "그래도 열기" 버튼 -->
